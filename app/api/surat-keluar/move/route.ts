...
export async function PATCH(req: Request) {
  try {
    ...
    if (type === "folder") {
      // Pindahkan FolderSuratKeluar ke folder orang tua lain
      const updatedFolder = await prisma.folderSuratKeluar.update({
        where: { id: id },
        data: { parentId: destinationId },
      });
      return NextResponse.json(updatedFolder);
    } else {
      // Pindahkan SuratKeluar ke FolderSuratKeluar lain
    ...
    console.error("Move Surat Keluar Error:", error);
