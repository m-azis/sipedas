...
interface DokumentasiItem {
  id: string;
  name?: string;       // Untuk folder
  namaDokumen?: string; // Untuk file dokumen
  type: "file" | "folder";
  tanggal?: string;
  tempat?: string;
  fileUrl?: string;
...
export default function DokumentasiDokumenKerja() {
  // State data khusus dokumentasi
  const [data, setData] = useState<DokumentasiItem[]>([]);
  const [loading, setLoading] = useState(true);
  
  // State untuk Navigasi Folder Dokumentasi
  ...
  // Memuat data khusus dari API Dokumentasi yang terpisah
  const loadData = async () => {
    setLoading(true);
    try {
      // Mengarah ke endpoint terisolasi: /api/dokumentasi
      const url = currentFolderId 
        ? `/api/dokumentasi?parentId=${currentFolderId}` 
        : "/api/dokumentasi?all=true";
      ...
      if (res.ok) {
        ...
        if (Array.isArray(d)) {
          ...
          const combined = [
            ...(d.folders || []).map((f: any) => ({ ...f, type: 'folder' as const })),
            ...(d.files || []).map((f: any) => ({ ...f, type: 'file' as const }))
          ];
          setData(combined);
        } else {
          setData(d.data || []);
      ...
      console.error("Gagal memuat data khusus dokumentasi:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string, type: string) => {
    const message = type === 'folder' 
      ? "Hapus folder dokumentasi ini? Semua isi di dalamnya mungkin tidak akan terlihat." 
      : "Apakah Anda yakin ingin menghapus data dokumentasi ini?";

    if (confirm(message)) {
      try {
        // Eksekusi hapus langsung ke tabel dokumentasi via API khusus
        const res = await fetch(`/api/dokumentasi?id=${id}&type=${type}`, { method: "DELETE" });
        if (res.ok) {
          alert("Data dokumentasi berhasil dihapus");
          loadData();
        } else {
          alert("Gagal menghapus data dari database dokumentasi");
        }
      } catch (error) {
  ...
  // LOGIKA SEARCH (Spesifik field Dokumentasi)
      ...
      <header style={{ 
        ...
        <div style={{ textAlign: 'left' }}>
          <h1 style={{ fontSize: '1.3rem', fontWeight: 900, color: '#800000', margin: 0 }}>
            DOKUMENTASI KREASIONAL ARSIP
          </h1>
          <p style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '4px', fontWeight: 600 }}>
            Dinas Pendidikan Kabupaten Bojonegoro &bull; Bidang Peningkatan Mutu Pendidikan Dasar
          </p>
        </div>
        <div style={{ display: 'flex', gap: '10px' }}>
          {currentFolderId && (
            ...
            >
              ⬅ KEMBALI KE ROOT DOKUMENTASI
            </button>
          )}
          <Link href="/dokumentasi/tambah" style={{ 
      ...
      <div style={{ padding: '2rem' }}>
        ...
        <div style={{ marginBottom: '1.5rem', display: 'flex', gap: '10px' }}>
          <div style={{ position: 'relative', flex: 1 }}>
            <input 
              type="text"
              placeholder="Cari arsip dokumentasi berdasarkan nama atau lokasi tempat..."
              value={searchTerm}
              onChange={(e) => {
        ...
        <div style={cardStyle}>
          <div style={cardHeaderStyle}>
            <span style={{ fontWeight: 800, fontSize: '0.85rem', color: '#475569' }}>
              {currentFolderId ? "DIREKTORI ARSIP DOKUMENTASI" : "DAFTAR REKAPITULASI DATA DOKUMENTASI"}
            </span>
            ...
            <table style={{ minWidth: '1000px', width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  ...
                  <th style={thStyle}>NAMA DOKUMEN DOKUMENTASI / FOLDER</th>
                  <th style={thStyle}>TANGGAL REKAP</th>
                  <th style={thStyle}>LOKASI / TEMPAT KEGIATAN</th>
                  <th style={thStyle}>FILE LAMPIRAN MEDIA</th>
                  <th style={{ ...thStyle, borderRight: 'none', textAlign: 'center' }}>AKSI</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr><td colSpan={6} style={statusTextStyle}>Menghubungkan ke database dokumentasi...</td></tr>
                ) : currentItems.length === 0 ? (
                  <tr><td colSpan={6} style={statusTextStyle}>Data dokumentasi tidak ditemukan atau folder kosong.</td></tr>
                ) : currentItems.map((item, index) => {
                  const isFolder = item.type === 'folder' || (!item.namaDokumen && item.name);
                  
                  // NORMALISASI BERKAS UNTUK DOWNLOAD/PREVIEW KHUSUS DIREKTORI DOKUMENTASI
                  let files: string[] = [];
                  if (item.fileUrl) {
                    ...
                    <tr key={item.id} style={{ background: index % 2 === 0 ? '#fff' : '#fafafa', borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={tdStyle}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                          {isFolder ? (
                            <span style={{ color: '#94a3b8', fontSize: '0.7rem', fontWeight: 700 }}>DIREKTORI FOLDER</span>
                          ) : files.length > 0 ? (
                            files.map((url, i) => {
                            ...
                            <span style={{ color: '#cbd5e1', fontSize: '0.7rem' }}>Tidak ada lampiran dokumentasi</span>
                          )}
                        </div>
                      </td>
                      <td style={{ ...tdStyle, borderRight: 'none', textAlign: 'center' }}>
                        <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
                          {!isFolder && (
                            <Link href={`/dokumentasi/edit/${item.id}`} style={{ color: '#2563eb', fontWeight: 800, textDecoration: 'none', fontSize: '0.7rem' }}>
                              EDIT
                            </Link>
                          )}
...
// INLINE STYLES (Pure Maroon & White CSS-in-JS Layout)
...
const fileBadgeStyle: React.CSSProperties = { background: '#fff1f2', color: '#800000', padding: '5px 10px', borderRadius: '4px', fontSize: '0.65rem', fontWeight: 800, textDecoration: 'none', border: '1px solid #fecaca', display: 'inline-flex', alignItems: 'center', gap: '6px', width: 'fit-content' };
const downloadBadgeStyle: React.CSSProperties = { background: '#f0fdf4', color: '#16a34a', padding: '5px 10px', borderRadius: '4px', fontSize: '0.65rem', fontWeight: 800, textDecoration: 'none', border: '1px solid #bbf7d0', display: 'inline-flex', alignItems: 'center', gap: '4px', width: 'fit-content', cursor: 'pointer' };
const btnPageStyle: React.CSSProperties = { padding: '8px 15px', borderRadius: '6px', border: '1px solid #e2e8f0', background: 'white', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer' };
const searchFieldStyle: React.CSSProperties = { width: '100%', padding: '12px 20px', borderRadius: '10px', border: '1px solid #e2e8f0', fontSize: '0.9rem', outline: 'none' };
...
const cardStyle: React.CSSProperties = { background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' };
