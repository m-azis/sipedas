...
export default function TambahDokumentasi() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Menggunakan metode POST untuk menambah data baru ke endpoint dokumentasi
    ...
    if (res.ok) { 
      alert("Data Dokumentasi Berhasil Ditambahkan!"); 
      ...
      <div className="form-card" style={{
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-body" style={{
            ...
            {/* Field Nama Dokumentasi */}
            <div className="input-group">
              <label style={labelStyle}>Nama / Judul Dokumentasi</label>
              <input 
                name="namaDokumen" 
                required 
                placeholder="Masukkan judul kegiatan atau nama dokumentasi" 
                style={inputStyle} 
              />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.8rem' }}>
              ...
              {/* Field Tempat */}
              <div className="input-group">
                <label style={labelStyle}>Tempat / Lokasi</label>
                <input 
                  name="tempat" 
                  required 
                  placeholder="Lokasi pengambilan dokumentasi" 
                  style={inputStyle} 
                />
              </div>
            </div>
            ...
            {/* Upload Section */}
            <div className="upload-container" style={{ background: '#fdf2f2', border: '2px dashed #f87171', padding: '2rem', borderRadius: '16px', marginTop: '10px' }}>
              <label style={{ display: 'block', marginBottom: '20px', color: '#800000', fontWeight: 900, fontSize: '0.85rem', textTransform: 'uppercase', textAlign: 'center' }}>
                📎 Unggah Berkas Dokumentasi (Foto/Video/PDF) (Opsional)
              </label>
              ...
              <div className="upload-mode-toggle" style={{ display: 'flex', gap: '15px', marginBottom: '1.5rem', background: 'rgba(255,255,255,0.5)', padding: '6px', borderRadius: '12px' }}>
                <button 
                  ...
                  style={{ ...toggleBtnStyle, background: uploadMode === 'file' ? '#800000' : 'transparent', color: uploadMode === 'file' ? 'white' : '#64748b' }}
                >
                  UPLOAD FILE
                </button>
                <button 
                  ...
                  style={{ ...toggleBtnStyle, background: uploadMode === 'folder' ? '#800000' : 'transparent', color: uploadMode === 'folder' ? 'white' : '#64748b' }}
                >
                  UPLOAD FOLDER
                </button>
              </div>
          ...
          <div className="form-footer" style={{ padding: '2rem 2.5rem', background: '#f8fafc', borderTop: '1px solid #f1f5f9', textAlign: 'center' }}>
            ...
            >
              {loading ? "SEDANG MENYIMPAN..." : "SIMPAN DATA DOKUMENTASI"}
            </button>
          </div>
        </form>
      </div>
