...
export default function ManajemenBerkasDokumentasi() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // Mengarah ke API Dokumentasi
      const res = await fetch(`/api/dokumentasi?parentId=${currentPath || "null"}`);
      ...
      const mappedFolders: FileItem[] = folders.map((f: any) => ({
        ...
        category: "Folder Dokumentasi",
        parentId: f.parentId
      }));

      const mappedFiles: FileItem[] = files.map((f: any) => ({
        id: f.id.toString(),
        name: f.namaDokumen || "Tanpa Judul", // Menggunakan field namaDokumen sesuai schema
        type: "file",
        updatedAt: new Date(f.createdAt).toLocaleDateString('id-ID'),
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      alert("Gagal membuat folder dokumentasi.");
    }
  };

  // LOGIKA MULTI UPLOAD TERURAI: Mengirim data terpisah per file agar tersimpan mandiri di database
  const handleUploadFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true);
    try {
      // Membuat list promise upload untuk dieksekusi secara paralel
      const uploadPromises = Array.from(files).map(async (file) => {
        const formData = new FormData();
        const cleanName = file.name.split(/[\\/]/).pop() || file.name;
        
        // Bersihkan nama file dari ekstensi untuk dijadikan judul dokumen utama
        const namaDokumenTanpaEkstensi = cleanName.substring(0, cleanName.lastIndexOf('.')) || cleanName;

        formData.append("files", file);
        formData.append("paths", cleanName);
        formData.append("folderId", currentPath || "null");
        formData.append("namaDokumen", namaDokumenTanpaEkstensi);
        formData.append("tanggal", new Date().toISOString());
        formData.append("tempat", "-");
        ...
        if (!res.ok) {
          ...
          throw new Error(`Gagal mengunggah ${file.name}: ${text}`);
        }
        return res;
      });

      // Menjalankan semua proses upload secara bersamaan
      await Promise.all(uploadPromises);
      alert(`Berhasil mengunggah ${files.length} berkas dokumentasi secara terpisah.`);
      loadData();
    } catch (err: any) {
      alert(err.message || "Terjadi kesalahan saat mengunggah beberapa berkas.");
    } finally {
      setLoading(false);
      e.target.value = "";
    }
  ...
  const handlePaste = async (targetFolderId: string | null) => {
    ...
    try {
      const movePromises = movingItems.ids.map(id => {
        ...
        return fetch("/api/dokumentasi/move", { // Pastikan endpoint PATCH ini tersedia
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ 
            id: id, 
  ...
  const onDrop = async (e: React.DragEvent, targetFolderId: string) => {
    ...
    try {
      const res = await fetch("/api/dokumentasi/move", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          id: draggedId, 
  ...
  const handleDelete = async (id: string, type: string) => {
    if (!confirm(`Hapus ${type === 'folder' ? 'folder' : 'berkas'} dokumentasi ini secara permanen?`)) return;
    
    setLoading(true);
    try {
      const url = type === "folder" 
        ? `/api/dokumentasi?id=${id}&type=folder` 
        : `/api/dokumentasi/${id}`;
  ...
  return (
    <div style={{ backgroundColor: '#f4f7fa', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', padding: '1.5rem 2rem', background: 'white', borderBottom: '1px solid #e2e8f0' }}>
        <div>
          <h1 style={{ fontSize: '1.2rem', fontWeight: 900, color: '#800000', margin: 0 }}>PENYIMPANAN BERKAS DOKUMENTASI</h1>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            ...
            <label style={btnSecondaryStyle}>
              📄 UPLOAD BERKAS <input type="file" hidden multiple onChange={handleUploadFile} />
            </label>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                ...
                <th style={{ ...thStyle, textAlign: 'left' }}>NAMA FOLDER / NAMA DOKUMENTASI</th>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={5} style={statusTextStyle}>Memuat data dokumentasi...</td></tr>
              ) : filteredItems.length === 0 ? (
                <tr><td colSpan={5} style={statusTextStyle}>Belum ada file di folder dokumentasi ini.</td></tr>
      ...
      {showCreateModal && (
        <div style={modalOverlayStyle}>
          <div style={modalContentStyle}>
            <h3 style={{ margin: '0 0 1rem 0', fontWeight: 800, color: '#1e293b' }}>FOLDER DOKUMENTASI BARU</h3>
            <input 
              type="text" 
              value={folderName} 
