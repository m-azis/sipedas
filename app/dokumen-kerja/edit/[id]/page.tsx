...
export default function EditDokumenKerja() {
  ...
  useEffect(() => {
    const fetchDokumen = async () => {
      try {
        // Mengambil data dari endpoint dokumen-kerja
        const res = await fetch(`/api/dokumen-kerja/${params.id}`);
        if (res.ok) {
          const data = await res.json();
          setFormData(data);
        }
      } catch (error) {
        console.error("Gagal mengambil data dokumen kerja:", error);
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Mengarah ke endpoint dokumen-kerja dengan method PUT
    const res = await fetch(`/api/dokumen-kerja/${params.id}`, { 
      method: "PUT", 
      body: dataToSend 
    });
    ...
    if (res.ok) { 
      alert("Perubahan Dokumen Kerja Berhasil Disimpan!"); 
      router.push("/dokumen-kerja/berkas"); 
      router.refresh();
    } else {
      alert("Gagal memperbarui data dokumen kerja.");
    }
    setLoading(false);
  };

  if (fetching) return <div style={{ padding: '4rem', textAlign: 'center', color: '#800000', fontWeight: 800 }}>MENGAMBIL DATA DOKUMEN KERJA...</div>;
    ...
    <div className="page-wrapper" style={{ 
      ...
      <div className="form-card" style={{
        ...
        <div style={{ padding: '1.8rem 2.5rem', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          <h2 style={{ fontWeight: 900, color: '#1e293b', fontSize: '1.3rem', margin: 0 }}>EDIT DATA DOKUMEN KERJA</h2>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ padding: '2.5rem', display: 'flex', flexDirection: 'column', gap: '1.8rem' }}>
            ...
            <div className="input-group">
              ...
              <input 
                ...
                style={inputStyle} 
                placeholder="Masukkan nama dokumen kerja..."
              />
            </div>
            ...
            <div style={{ background: '#fdf2f2', border: '2px dashed #f87171', padding: '2rem', borderRadius: '16px' }}>
              ...
              <div style={{ textAlign: 'center' }}>
                <p style={{fontSize: '0.75rem', fontWeight: 700, color: '#991b1b', marginBottom: '12px'}}>
                  Kosongkan jika tidak ingin mengubah berkas yang sudah ada
                </p>
                <input 
          ...
          <div style={{ padding: '2rem 2.5rem', background: '#f8fafc', borderTop: '1px solid #f1f5f9', textAlign: 'center' }}>
            ...
            >
              {loading ? "MENYIMPAN PERUBAHAN..." : "UPDATE DATA DOKUMEN KERJA"}
            </button>
          </div>
        </form>
      </div>
