...
export default function ManajemenBerkasDokumenKerja() {
  ...
  // Memuat data arsip dokumen kerja dari API
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/dokumen-kerja?parentId=${currentPath || "null"}`);
      ...
      const mappedFiles: FileItem[] = files.map((f: any) => ({
        id: f.id.toString(),
        name: f.namaDokumen || f.perihal || "Tanpa Judul",
        type: "file",
        updatedAt: new Date(f.createdAt).toLocaleDateString('id-ID'),
      ...
      console.error("Gagal memuat data dokumen kerja:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // HANDLER BULK UPLOAD TERPISAH (Satu Berkas = Satu Record Database)
    ...
    try {
      const uploadPromises = Array.from(files).map(async (file) => {
        ...
        if (!res.ok) {
          ...
          let errorMsg = `Gagal mengunggah berkas: ${file.name}`;
          try {
  ...
  const handlePaste = async (targetFolderId: string | null) => {
    ...
    try {
      const movePromises = movingItems.ids.map(id => {
        ...
        return fetch("/api/dokumen-kerja/move", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ 
            id: id, 
  ...
  const onDrop = async (e: React.DragEvent, targetFolderId: string) => {
    ...
    try {
      const res = await fetch("/api/dokumen-kerja/move", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          id: draggedId, 
  ...
  const handleDelete = async (id: string, type: string) => {
    ...
    try {
      const url = type === "folder" 
        ? `/api/dokumen-kerja?id=${id}&type=folder` 
        : `/api/dokumen-kerja/${id}`;
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                ...
                <th style={{ ...thStyle, textAlign: 'left' }}>NAMA FOLDER / JUDUL DOKUMEN KERJA</th>
