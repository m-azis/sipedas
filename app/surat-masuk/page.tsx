...
// PERBAIKAN 1: Kembalikan impor Link ke next/link
import Link from "next/link"; 

export default function MonitoringSuratMasuk() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // Endpoint disesuaikan ke surat-masuk
      const url = currentFolderId 
        ? `/api/surat-masuk?parentId=${currentFolderId}` 
        : "/api/surat-masuk?all=true";
      ...
      console.error("Gagal load data surat masuk:", error);
    } finally {
      // PERBAIKAN 2: Ubah dari loading(false) menjadi setLoading(false)
      setLoading(false); 
    }
  };
  ...
  const handleDelete = async (id: any, type: string) => {
    const message = type === 'folder' 
      ...
      : "Apakah Anda yakin ingin menghapus arsip surat masuk ini?";

    if (confirm(message)) {
      try {
        const res = await fetch(`/api/surat-masuk?id=${id}&type=${type}`, { method: "DELETE" });
        if (res.ok) {
  ...
  // LOGIKA SEARCH: Mencari berdasarkan kolom utama surat masuk
  const filteredData = data.filter((item: any) => {
    const noBerkas = item.noBerkas || "";
    const pengirim = item.alamatPengirim || item.asalSurat || "";
    const nomor = item.nomorSurat || "";
    const perihal = item.perihal || item.name || ""; 
    
    const searchStr = `${noBerkas} ${pengirim} ${nomor} ${perihal}`.toLowerCase();
    ...
    <div style={{ backgroundColor: '#f4f7fa', minHeight: '100vh' }}>
      <header className="header-bar" style={{ 
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <Link href="/surat-masuk/tambah" style={{ 
      ...
      <div className="card-container" style={{ padding: '2rem' }}>
        <div style={{ marginBottom: '1.5rem', display: 'flex', gap: '10px' }}>
          <div style={{ position: 'relative', flex: 1 }}>
            <input 
              type="text"
              placeholder="Cari arsip masuk (No Berkas, Pengirim, Nomor Surat, Perihal...)"
              value={searchTerm}
              onChange={(e) => {
        ...
        <div className="data-card" style={cardStyle}>
          <div style={cardHeaderStyle}>
            <span style={{ fontWeight: 800, fontSize: '0.85rem', color: '#475569' }}>
              {currentFolderId ? "ISI FOLDER SURAT MASUK" : "DAFTAR ARSIP MASUK DIGITAL (SELURUHNYA)"}
            </span>
            ...
            <table style={{ minWidth: '1500px', width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  ...
                  <th style={thStyle}>ALAMAT PENGIRIM / NAMA</th>
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={10} style={statusTextStyle}>Sinkronisasi database masuk...</td></tr>
                ) : currentItems.length === 0 ? (
                  <tr><td colSpan={10} style={statusTextStyle}>Data surat masuk tidak ditemukan.</td></tr>
                ) : currentItems.map((item: any, index: number) => {
                  // Identifikasi apakah ini folder atau file berdasarkan tipe atau properti nomorSurat
                  ...
                  return (
                    <tr key={item.id} style={{ background: index % 2 === 0 ? '#fff' : '#fafafa', borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ ...tdStyle, fontWeight: 700, color: isFolder ? '#2563eb' : '#334155' }}>
                        ...
                        ) : (item.alamatPengirim || item.asalSurat || "-")}
                      </td>
                      ...
                      <td style={{ ...tdStyle, fontSize: '0.75rem', color: '#64748b', lineHeight: '1.5', maxWidth: '250px' }}>
                        {isFolder ? "Folder Penyimpanan Surat Masuk" : item.perihal}
                      </td>
                      <td style={tdStyle}>
                      ...
                      <td style={{ ...tdStyle, borderRight: 'none', textAlign: 'center' }}>
                        <div style={{ display: 'flex', justifyContent: 'center', gap: '12px' }}>
                          {!isFolder && (
                            <Link href={`/surat-masuk/edit/${item.id}`} style={{ color: '#2563eb', fontWeight: 800, textDecoration: 'none', fontSize: '0.7rem' }}>
                              EDIT
                            </Link>
                          )}
...
// Style untuk Tombol Download Berkas (Konsisten & Kontras Hijau Lembut)
