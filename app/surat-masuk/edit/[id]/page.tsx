...
export default function EditSuratMasuk() {
  ...
  // Fungsi pembantu untuk memformat tanggal ke YYYY-MM-DD
  ...
  if (fetching) return <div style={{ padding: '4rem', textAlign: 'center', color: '#800000', fontWeight: 800 }}>MENGAMBIL DATA ARSIP...</div>;
    ...
    <div className="page-wrapper" style={{ 
      ...
      <div className="form-card" style={{
        ...
        <div style={{ padding: '1.8rem 2.5rem', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          <h2 style={{ fontWeight: 900, color: '#1e293b', fontSize: '1.3rem', margin: 0 }}>EDIT DATA SURAT MASUK</h2>
        </div>

        <form onSubmit={handleSubmit}>
          <div style={{ padding: '2.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.8rem' }}>
            
            <div className="input-group">
              <label style={labelStyle}>Nomor Urut</label>
              <input name="noUrut" type="number" required defaultValue={formData.noUrut} style={inputStyle} />
            </div>
            
            <div className="input-group">
              <label style={labelStyle}>Nomor Berkas</label>
              <input name="noBerkas" required defaultValue={formData.noBerkas} style={inputStyle} />
            </div>

            <div style={{ gridColumn: 'span 2' }}>
              <label style={labelStyle}>Alamat Pengirim</label>
              <input name="alamatPengirim" required defaultValue={formData.alamatPengirim} style={inputStyle} />
            </div>

            <div className="input-group">
              <label style={labelStyle}>Nomor Surat</label>
              <input name="nomorSurat" required defaultValue={formData.nomorSurat} style={inputStyle} />
            </div>

            <div className="input-group">
              <label style={labelStyle}>Tanggal Surat</label>
              {/* PERBAIKAN: Menggunakan fungsi formatDateForInput agar tanggal muncul otomatis */}
              <input 
                name="tanggalSurat" 
                type="date" 
                required 
                defaultValue={formatDateForInput(formData.tanggalSurat)} 
                style={inputStyle} 
              />
            </div>

            <div className="input-group">
              <label style={labelStyle}>No. Petunjuk (P)</label>
              <input name="noPetunjuk" defaultValue={formData.noPetunjuk} style={inputStyle} />
            </div>

            <div className="input-group">
              <label style={labelStyle}>No. Paket (K)</label>
              <input name="noPaket" defaultValue={formData.noPaket} style={inputStyle} />
            </div>

            <div style={{ gridColumn: 'span 2' }}>
              <label style={labelStyle}>Perihal Surat</label>
              <textarea name="perihal" rows={4} required defaultValue={formData.perihal} style={{ ...inputStyle, resize: 'vertical' }}></textarea>
            </div>

            {/* Upload Section */}
            <div style={{ gridColumn: 'span 2', background: '#fdf2f2', border: '2px dashed #f87171', padding: '2rem', borderRadius: '16px' }}>
              <label style={{ display: 'block', marginBottom: '20px', color: '#800000', fontWeight: 900, fontSize: '0.85rem', textAlign: 'center' }}>
                📎 UPDATE LAMPIRAN DIGITAL (OPSIONAL)
              </label>
              ...
              <div style={{ textAlign: 'center' }}>
                <p style={{fontSize: '0.75rem', fontWeight: 700, color: '#991b1b', marginBottom: '12px'}}>
                  Kosongkan jika tidak ingin mengubah lampiran yang sudah ada
                </p>
                <input 
          ...
          <div style={{ padding: '2rem 2.5rem', background: '#f8fafc', borderTop: '1px solid #f1f5f9', textAlign: 'center' }}>
            ...
            >
              {loading ? "MENYIMPAN PERUBAHAN..." : "UPDATE DATA ARSIP"}
            </button>
          </div>
        </form>
      </div>
