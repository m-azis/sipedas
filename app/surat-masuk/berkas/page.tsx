...
export default function ManajemenBerkasSuratMasuk() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/surat-masuk?parentId=${currentPath || "null"}`);
  ...
  const handleUploadFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const fileAny = files[i] as any;
      // Perbaikan: Ambil hanya nama file saja untuk menghindari ENOENT path dari browser
      const cleanName = files[i].name.split(/[\\/]/).pop() || files[i].name;
    ...
    formData.append("asalSurat", "-"); 
    formData.append("tanggalSurat", new Date().toISOString());
    formData.append("nomorSurat", "-");
    formData.append("perihal", files.length > 1 ? `Upload ${files.length} berkas` : files[0].name); 
  ...
  const handlePaste = async (targetFolderId: string | null) => {
    ...
    try {
      const movePromises = movingItems.ids.map(id => {
        ...
        return fetch("/api/surat-masuk/move", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ 
            id: id, 
  ...
  const onDrop = async (e: React.DragEvent, targetFolderId: string) => {
    ...
    try {
      const res = await fetch("/api/surat-masuk/move", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          id: draggedId, 
  ...
  // PERBAIKAN TOTAL FUNGSI DELETE
  const handleDelete = async (id: string, type: string) => {
    if (!confirm(`Apakah Anda yakin ingin menghapus ${type === 'folder' ? 'folder' : 'file'} ini?`)) return;
    
    setLoading(true);
    try {
      // Pastikan URL mengarah ke endpoint yang benar sesuai struktur API Route Anda
      const url = type === "folder" 
        ? `/api/surat-masuk?id=${id}&type=folder` 
        : `/api/surat-masuk/${id}`;
      ...
      if (res.ok) {
        ...
        // Ambil text dulu untuk menghindari "Unexpected end of JSON"
  ...
  return (
    <div style={{ backgroundColor: '#f4f7fa', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', padding: '1.5rem 2rem', background: 'white', borderBottom: '1px solid #e2e8f0' }}>
        <div>
          <h1 style={{ fontSize: '1.2rem', fontWeight: 900, color: '#800000', margin: 0 }}>PENYIMPANAN DIGITAL SURAT MASUK</h1>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            ...
            <label style={btnSecondaryStyle}>
              ...
              <input 
                type="file" 
                hidden 
                /* @ts-ignore: webkitdirectory and directory are not in standard Typescript React types */
                webkitdirectory="" 
                directory="" 
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                ...
                <th style={{ ...thStyle, textAlign: 'left' }}>NAMA FOLDER / PERIHAL SURAT</th>
                ...
                >
                  ...
                  <td style={{ ...tdStyle, borderRight: 'none' }}>
                    <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
                      {item.type === 'file' && (
                        <>
                          <Link href={`/surat-masuk/edit/${item.id}`} style={actionLinkBlue}>EDIT</Link>
                          <a href={item.fileUrl?.startsWith('http') ? item.fileUrl : `https://vbgygthhazkecogdleyd.supabase.co/storage/v1/object/public/surat-masuk/${item.fileUrl}`} target="_blank" rel="noopener noreferrer" style={{ color: '#059669', fontWeight: 800, textDecoration: 'none', fontSize: '0.7rem' }}>LIHAT</a>
                        </>
                      )}
                      <button 
